...
def network_to_pp_json(network: Network) -> str:
    ...
    net, id_maps = build_net(network, require_slack=False)
    ...
    net["diagram_ext_grid"] = pd.DataFrame(
        [
            {
                "uuid": uid,
                "x": gen_by_id[uid].x,
                "y": gen_by_id[uid].y,
                "waypoint_json": _waypoint_json(gen_by_id[uid].waypoint),
            }
            for uid in id_maps["ext_grid"]
        ],
        index=list(id_maps["ext_grid"].values()),
    )
...
def pp_json_to_network(raw: str) -> Network:
    ...
    Only bus / ext_grid / load are reconstructed for now; other element tables
    (line, trafo, gen, sgen, …) are ignored until the editor supports them.
    """
    ...
    for j in net.ext_grid.index:
        ...
        generators.append(
            Generator(
                ...
                name=_name(net.ext_grid.at[j, "name"], "Generator"),
