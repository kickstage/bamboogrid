...
def two_bus_with_switch(closed: bool) -> Network:
    # Gen on b1, load on b2, joined by one bus-bus switch.
    return Network(
        id="n",
        name="sw",
        buses=[Bus(id="b1", vn_kv=0.4), Bus(id="b2", vn_kv=0.4)],
        generators=[Generator(id="g1", bus_id="b1", vm_pu=1.0)],
...
def test_open_switch_leaves_load_island_without_slack():
...
def test_unwired_switch_is_ignored():
    net = Network(
        ...
        generators=[Generator(id="g1", bus_id="b1")],
